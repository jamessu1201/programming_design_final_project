...
上面兩個指令只補「每日明細」，`/points top` 的累計總數不會動——因為點數系統
上線之後的訊息早就一則一則加進去了，再加一次就是重複計算。要把上線**之前**的
歷史併進總數，跑：

```
!points_apply_history 2026-06-15 dry   # 換成你的點數系統上線日，先預覽
!points_apply_history 2026-06-15
```

它只取該日期**之前**的明細，寫進獨立的 `history_points` / `history_messages`
欄位（覆寫而非累加，所以重跑結果相同），顯示的總數 = 即時累積 + 這兩個欄位。
